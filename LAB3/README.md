...
* **YouTube:** [Xem video thực hành LAB 3](https://youtu.be/M1myTt6G09Q)
...
* **Phạm vi kiểm thử tải:** Chỉ thực hiện trên địa chỉ cục bộ `127.0.0.1:8080`

Thông tin phiên bản chi tiết của hệ điều hành và các công cụ được trình bày trong báo cáo Word.
...
8. Tạo mã SHA-256 để kiểm tra tính toàn vẹn của các tệp bằng chứng.
...
* Phân biệt tài sản, lỗ hổng, mối đe dọa, rủi ro và hành vi tấn công.
...
* Theo dõi hoạt động và các kết nối trong máy thực hành.
* Thu thập output phục vụ quá trình phân tích.
* **Kết quả:** PASS
...
* Lưu kết quả và ảnh chụp làm bằng chứng.
* **Kết quả:** PASS

### TH5 – Kiểm thử tải cục bộ

* Chạy máy chủ thử nghiệm trên địa chỉ `127.0.0.1:8080`.
...
| Tình huống | Nội dung thực hiện                        | Kết quả |
| ---------- | ----------------------------------------- | :-----: |
| TH1        | Nhận diện các khái niệm an toàn thông tin |   PASS  |
| TH2        | Kiểm tra Microsoft Defender bằng EICAR    |   PASS  |
| TH3        | Quan sát tiến trình và kết nối hệ thống   |   PASS  |
| TH4        | Phân tích lưu lượng mạng bằng Wireshark   |   PASS  |
...
* **Wireshark hiển thị quá nhiều gói tin:** Sử dụng bộ lọc để thu hẹp dữ liệu cần phân tích.
...
* **Một số log chứa thông tin của máy thực hành:** Làm sạch tên máy, tên tài khoản, địa chỉ IP và dữ liệu nhạy cảm trước khi tải lên.
...
LAB3/
├── README.md
├── 11_ĐH_CNPM2-LAB3_1150080130-LeCongDat.docx
├── evidence_sha256.csv
├── output/
└── logs/
```
...
Tệp `evidence_sha256.csv` chứa mã SHA-256 của các tệp bằng chứng. Mã SHA-256 được sử dụng để kiểm tra các tệp có bị thay đổi sau khi thu thập hay không.
